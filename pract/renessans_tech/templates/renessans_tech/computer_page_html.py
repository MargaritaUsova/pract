...
with open('computer_page.html', 'w') as f:
    f.write("""
    <!DOCTYPE html>
{% load static %}
<html lang="ru">
...
<header class="site-header">
  ...
  <script>
    ...
    for i in prices:
        f.write("""
    if (currentUrl == "http://127.0.0.1:8000/computers/{link}"){{
      ...
      document.getElementById("feature-info-2").innerHTML = "{d}";
      ...
      document.getElementById("item-photo-3").src = "{{% static '{photo3}' %}}";
            ...
            """.format(link=prices[i]['nameTranslit'],
                       ...
                       d = prices[i]['Диагональ/разрешение'],
                       proc = prices[i]['Тип процессора'],
                       ram = prices[i]['Оперативная память (RAM)'],
                       graf = prices[i]['Графический контроллер'],
